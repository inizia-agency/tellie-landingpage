  ...
  return (
    <>
      ...
      <aside className="group/nav fixed inset-y-0 left-0 z-50 hidden w-[76px] flex-col overflow-hidden border-r border-stone-200 bg-white shadow-[8px_0_30px_rgba(34,34,34,.05)] transition-[width] duration-300 hover:w-[270px] focus-within:w-[270px] md:flex">
        ...
        <nav aria-label="Portal navigation" className="portal-nav-scroll mt-3 flex-1 space-y-1 overflow-y-auto overflow-x-hidden px-3 pb-3">
          {portalTabs.map(([id, label]) => { const Icon = portalIcons[id]; return <button title={label} key={id} onClick={() => showPanel(id, setActive)} aria-current={active === id ? 'page' : undefined} className={`focus-ring flex h-11 w-full items-center justify-center overflow-hidden rounded-xl px-0 text-sm font-medium transition group-hover/nav:justify-start group-hover/nav:px-[14px] group-focus-within/nav:justify-start group-focus-within/nav:px-[14px] ${active === id ? 'bg-[#f4eef8] text-[#75438a]' : 'text-stone-600 hover:bg-stone-100 hover:text-stone-900'}`}><Icon className="h-5 w-5 shrink-0" /><span className="ml-4 whitespace-nowrap opacity-0 transition-opacity duration-200 group-hover/nav:opacity-100 group-focus-within/nav:opacity-100">{label}</span></button>; })}
        </nav>
